...
class CreateYNABTransactionFromNuBankNotificationTest {
    ...
    private val configurationGateway: ConfigurationGatewayInMemory = ConfigurationGatewayInMemory()
    ...
    init {
        this.usecase = CreateYNABTransactionFromNuBankNotification(
                this.YNABGateway,
                this.payeeToCategoryGateway,
...
//    @Test
    ...
    @Test
    fun `Send to NuConta account when message is about debit which doesnt have APROVADA text`() {
        this.configurationGateway.put("NuContaAccountID", "NUCONTA")

        this.usecase.create("com.nu.production", "Compra de R$ 1.234,56 em Nice Looking Company.")

        assertEquals(1, YNABGateway.transactions.count())
        assertEquals("NUCONTA", YNABGateway.transactions.get(0).accountId)
    }

    @Test
    fun `Send to NuBank account when message is about credit card which have APROVADA text`() {
        this.configurationGateway.put("NuBankAccountID", "NUBANK")

        this.usecase.create("com.nu.production", "Compra de R$ 1.234,56 APROVADA em Nice Looking Company.")

        assertEquals(1, YNABGateway.transactions.count())
        assertEquals("NUBANK", YNABGateway.transactions.get(0).accountId)
    }
}